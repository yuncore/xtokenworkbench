        ...
        watch: {
            filterActiveGroup(val){
                if(this.filterActiveGroup.length > 0){
                    ...
                    this.getCurrencyList(this.currentPage, this.currentNum, this.currentOrder, this.currentProp)
                }
            },
            filterActiveTag(val){
        ...
        methods: {
            ...
            groupChooseConfirm(){
                if(this.groupChooseCurrencyId === ''){
                    ...
                    this.$alert(`确定添加分组: ${this.groupChooseCurrencyName} => ${str_groups}?`, '添加分组', {
                        confirmButtonText: '确定'
                    }).then(() => {
            ...
            calcGroupNameById(ids){
                let chosenGroup = this.groupList.filter(item => ids.indexOf(item.id) > -1);
            ...
            tagChooseConfirm(){
                if(this.tagChooseCurrencyId === ''){
                    ...
                    this.$alert(`确定设置标签: ${this.tagChooseCurrencyName} => ${str_tags}?`, '关联标签', {
                        confirmButtonText: '确定'
                    }).then(() => {
...
//                    this.$message({type: 'error', message: res.responseJSON.message || '未知错误'})
            ...
            unSelectFilterItem (item) {
                ...
                this.getCurrencyList(1, this.currentNum, this.currentOrder, this.currentProp)
            },
            // 选择列表过滤字段的回调
            ...
            handlePageChange (val) {
                if(val !== this.currentPage){
                    this.getCurrencyList(val, this.currentNum, this.currentOrder, this.currentProp)
                }
            },
            // 货币列表单页显示条数改变时的回调
