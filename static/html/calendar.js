...
var calender = new function (){
	...
	function initCalendar(id,evn){
		var calendarHtml = '<div class="calendar"><p class="calendar-year" id="pcalyear">2015年</p><a href="javascript:calender.setLastMonth();" class="calendar-btn calendar-btn-l"><span class="icon-triangle-w"><</span></a><a href="javascript:calender.setNextMonth();" class="calendar-btn  calendar-btn-r"><span class="icon-triangle-e">></span></a><div class="calendar-months"><ul class="calendar-months calendar-monthsZ" id="calmonthli"><li>1月</li><li>2月</li><li>3月</li><li class="months-cur">4月</li><li>5月</li><li>6月</li><li>7月</li><input type="hidden" value="" id="hidyear0" /><input type="hidden" value="" id="hidyear1" /><input type="hidden" value="" id="hidyear2" /><input type="hidden" value="" id="hidyear3" /><input type="hidden" value="" id="hidyear4" /><input type="hidden" value="" id="hidyear5" /><input type="hidden" value="" id="hidyear6" /></ul></div><div class="calendar-day" ><ul class="week week-b week-hd" ><li>日</li><li>一</li><li>二</li><li>三</li><li>四</li><li>五</li><li>六</li></ul><ul class="week week-b week-day week-day-b" id="caldayli"><li></li><li></li><li></li><li></li><li></li><li></li><li class="duty-full"><span>2</span><p>全天</p></li></ul></div></div><div  class="title-full" id="disDate"></div><input type="hidden" value="" id="selectYear"><input type="hidden" value="" id="selectMonth"><input type="hidden" value="" id="selectDay"><input type="hidden" value="3" id="selectMonIndex">';
	...
	/*	for(var i=0;i<7;i++){
			...
			if((m*1+1) == 12)
				...
				$("#calmonthli li").eq(i).html((m*1+1)%12+"月");//设置当月li
		}*/
		//改变选择月
		var selectMonth = $("#selectMonth").val();
		$("#selectMonth").val((selectMonth*1+1)%12);
		//改变选择年
	...
	function setLastMonth(){
		/*for(var i=0;i<7;i++){
			...
			if((m*1-1+12) == 12)
				...
				$("#calmonthli li").eq(i).html((m*1-1+12)%12+"月");//设置当月li
		}*/


		//改变选择月
		var selectMonth = $("#selectMonth").val();
		$("#selectMonth").val((selectMonth*1-1+12)%12);
		//改变选择年
		//var index = $("#selectMonIndex").val();//获取选中月下标
		//var syear = $("#hidyear"+index).val();//获取选中月对应年
		//改变选择年
	...
	function setDisDate(){
		...
		//$("#selectYear").val()+"年 "+($("#selectMonth").val()*1+1)+"月 "+$("#selectDay").val()+"日  星期" +getweek(getDisDate().getDay())
	}
	/*
	 * 设置显示日期
	 * return: 当前选择日期
	...
	function setTitle(){
	   //$("#pcalyear").html($("#selectYear").val()+"年 "+($("#selectMonth").val()*1+1)+"月 "+$("#selectDay").val()+"日  星期" +getweek(getDisDate().getDay()));//改变显示年}
		$("#pcalyear").html($("#selectYear").val()+"年 "+($("#selectMonth").val()*1+1)+"月 ");
	}
}
