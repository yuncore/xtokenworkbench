;(function (e, t, n) {
    ...
    3: [function (require, module, exports) {
        ...
        canvas.addEventListener('click', function (e) {
            var x = e.pageX || customerPageX,
                y = e.pageY || customerPageY,
        ...
        ticker(window, 60).on('tick', function () {
            ...
            // ctx.fillStyle = 'rgba(22,35,45,0.8)';
            ctx.fillStyle = 'rgb(27, 33, 47)'
            // var radialGradient1 = ctx.createRadialGradient(canvas.width/2, canvas.height/2, 200, canvas.width/2, canvas.height/2,  canvas.width/2);
            // radialGradient1.addColorStop(0, 'rgba(57,60,76,0.25)');
            // radialGradient1.addColorStop(1, 'rgba(36,36,46,0.25)');
            // ctx.fillStyle = radialGradient1;
            ctx.fillRect(0, 0, canvas.width, canvas.height)

            // ctx.fillStyle = '#543D5E';
            ctx.fillStyle = 'rgb(7, 88, 134)'
            var radius = 6
            for (var i = 0, l = boidData.length, x, y; i < l; i += 1) {
                ...
                // ctx.fillRect(x + halfWidth, y + halfHeight, 3, 3);
                var sx = boidData[i].speed.x
                var sy = boidData[i].speed.y
                var heading = Math.acos(sx / Math.sqrt(sx * sx + sy * sy))
                if (sy < 0) {
                    heading = -heading
                }
                ctx.beginPath()
                ctx.moveTo(x + halfWidth + Math.cos(heading) * radius, y + halfHeight + Math.sin(heading) * radius)
                ctx.lineTo(x + halfWidth + Math.cos(heading + 5 * Math.PI / 6) * radius, y + halfHeight + Math.sin(heading + 5 * Math.PI / 6) * radius)
                ctx.lineTo(x + halfWidth + Math.cos(heading + 7 * Math.PI / 6) * radius, y + halfHeight + Math.sin(heading + 7 * Math.PI / 6) * radius)
                ctx.fill()
            }
        })
...
// var countText = document.querySelector('[data-count]');
    ...
    }, {'./vector': 1, './boid': 2, './': 4, 'ticker': 5, 'fps': 6, 'debounce': 7}],
...
// EventEmitter.prototype.emit() is also defined there.
    ...
    4: [function (require, module, exports) {
        ...
        function Boids (opts, callback) {
            ...
            for (var i = 0, l = opts.boids === undefined ? 15 : opts.boids; i < l; i += 1) {
                boids[i] = new Boid(
    ...
    }, {'events': 9, './vector': 1, './dtree': 10, './boid': 2, 'inherits': 11}],
